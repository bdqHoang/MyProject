...
namespace MyProject.Application.Features.Message.Queries
{
        ...
        ) : IRequestHandler<GetMessageQuery, List<MessageRes>>
    {
        public async Task<List<MessageRes>> Handle(GetMessageQuery request, CancellationToken cancellationToken)
        {
            ...
            var isPaticiant = await unitOfWork.ParticipantRepository.IsUserInConversationAsync(request.ConversationId, request.UserId);
            if (!isPaticiant)
            {
            ...
            var messages = await unitOfWork.MessageRepository.GetMessagesByConversationAsync(request.ConversationId, request.Page, request.PageSize);
            var messageRes = mapper.Map<List<MessageRes>>(messages);

            // update last seen item
            await unitOfWork.ParticipantRepository.UpdateLastSeenAsync(request.ConversationId, request.UserId);
            return messageRes;
        }
    }
}
